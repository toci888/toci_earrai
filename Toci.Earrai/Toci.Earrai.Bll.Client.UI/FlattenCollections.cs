...
namespace Toci.Earrai.Bll.Client.UI
{
    public class FlattenCollections : FlattenBase
    {
        public FlattenCollections()
        {
            ...
            FlattenMap.Add("AreasQuantities", (ob, input) => { input.AddRange(Flatten(((ProductDto)ob).AreaQuantities, (obj) => new FlattenedEntity() { Name = "Area - Quantity - Date", Value = (obj).Areaname + " - " + (obj).Quantity + " - " + (obj).Createdat.Value.ToString(ConstsUI.DateFormat) })); return input; });
            FlattenMap.Add("QuotesAndPrices", (ob, input) => { input.AddRange(Flatten(((ProductDto)ob).Quotesandprices, (obj) => new FlattenedEntity() { Name = "Valuation - Price - Vendor - Date", Value = (obj).Valuation + " - " + (obj).Price + " - " + (obj).Vendor + " - " + (obj).Createdat.Value.ToString(ConstsUI.DateFormat) })); return input; });
